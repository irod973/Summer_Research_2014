...
def drawMixture(domParams, contParams, skyMap, pixelScale=.2, stampSize=100,  sky=False, psf=False):
	...
	# print "The flux above threshold: ", fluxAboveThresh
	# print "Total flux: ", domParams['flux'].value
	if sky==True:
...
for sep in sepRange:
	contParams['centX'].value = sep*np.cos(phi)
	contParams['centY'].value = sep*np.sin(phi)
	e1, e2, e1err, e2err, e1A, e2A = calcEllipticities(domParams, contParams, skyMap,sky=True)
	valList = [e1, e2, e1err, e2err, e1A, e2A]
...
#The things below are to study noise bias from sky by look at the pull distributions of the fit ellipticities with and without sky
